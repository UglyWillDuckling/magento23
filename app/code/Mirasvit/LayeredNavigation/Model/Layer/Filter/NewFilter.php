...
class NewFilter extends AbstractFilter
{
     ...
     * @var string
     */
    protected $attributeCode = AdditionalFiltersConfigInterface::NEW_FILTER;
    ...
    public function __construct(
        ...
        $this->_requestVar = AdditionalFiltersConfigInterface::NEW_FILTER_FRONT_PARAM;
    ...
    {
        if (!$this->additionalFiltersConfig->isNewFilterEnabled($this->storeManager->getStore()->getStoreId())) {
            return $this;
        }
        $filter = $request->getParam(AdditionalFiltersConfigInterface::NEW_FILTER_FRONT_PARAM);

        if ($filter) {
            $this->filterNewService->addNewFilterInCollection($this->getLayer()->getProductCollection());
            $this->addState(__('Yes'), $filter);
            $this->isAdded = true;
        }
    ...
    {
        $newName = $this->additionalFiltersConfig->getNewFilterLabel($this->storeManager->getStore()->getStoreId());
        $newName = ($newName) ? : AdditionalFiltersConfigInterface::NEW_FILTER_DEFAULT_LABEL;
