...
class Wished implements IndexerActionInterface, MviewActionInterface
{
    /**
    ...
    public function execute($ids)
    {
        ...
        $table      = $this->resource->getTableName(WishedCriterion::TABLE_NAME);
        $select     = $connection->select();

        $select->from($this->resource->getTableName('wishlist_item'), [
                'product_id',
                'store_id',
                WishedCriterion::COUNT => new \Zend_Db_Expr('COUNT(*)')
            ])
            ->where('wishlist_item_id IN(?)', $ids)
            ->group('product_id');
