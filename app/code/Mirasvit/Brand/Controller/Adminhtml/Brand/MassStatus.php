...
class MassStatus extends Brand
{
    ...
    {
        $status = $this->getRequest()->getParam(BrandPageInterface::IS_ACTIVE);
        ...
        if ($ids) {
            try {
                foreach ($ids as $id) {
                    $model = $this->brandPageRepository->get($id)->setIsActive($status);
                    $this->brandPageRepository->save($model);
                }
                $this->messageManager->addSuccessMessage(
                    __('%1 item(s) have changed status', count($ids))
                );
            } catch (\Exception $e) {
